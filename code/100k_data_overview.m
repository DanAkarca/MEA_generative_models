%% tSNE overview of 100k recording data
% written by Manuel Schroter 
% adapted by Danyal Akarca
%% set pre-requisites
clear all; close all
% set code path
code_path = '/imaging/astle/users/da04/PhD/hd_gnm_generative_models/toolboxes/';
% add code paths
addpath(genpath(strcat(code_path,'AC_clustering_figure6/')));
addpath(genpath(strcat(code_path,'CellExplorer-master/')));
% set data path
data_path = '/imaging/astle/users/da04/PhD/hd_gnm_generative_models/ToShare_Jan22/';
...
%% visualise
h = figure('Units', 'pixels', 'Position', [100 100 900 900],'color','white');
[grid,cc,cmap,xrng,yrng,himage] = plot_smoothed_cluster(Y,A,indices,cmap);
xlabel('tSNE 1'); ylabel('tSNE 2');
xticks([]); yticks([]);
box off;
set(gca, 'TickDir', 'out','TickLength', [.01 .01],'linew',1,'FontSize',25,'FontName','Arial');
%{
...
%{
figure; 
subplot(121);gscatter(Y(:,1),Y(:,2),indices);
subplot(122);imagesc(mat);
% figure
 subplot(151);plot(rodent_table_ccg_unity')
 title('rodent')
 subplot(152);plot(gluta_table_ccg_unity')
 title('gluta')
 subplot(153),plot(dopa_table_ccg_unity')
 title('dopa')
 subplot(154),plot(moto_table_ccg_unity')
 title('moto')
 subplot(155),plot(org_table_ccg_unity')
 title('organoids')
%}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
