%% Run voronoi tesselation of 100k rodent primary cortical neuronal cultures
...
function output = run_rodent_100k_voronoi(pipeline,network,div);
%% add relevant paths
...
% set save directory
sdir = '/imaging/astle/users/da04/PhD/hd_gnm_generative_models/rodent_qc/';
...
for model = 1:nmodels;
    % print text
    disp(sprintf('running subject %g model %s...',network,modeltypes(model)));
    % run the model
...
cd(sdir);
% save file
save(sprintf('rodent_100k_qc_%g_%g_%g_generative_model.mat',pipeline,network,div),'output','-v7.3');
end
